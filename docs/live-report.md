...
> **现状：暂时不迁 Cloudflare（Pages 那条路没走通），继续用 GitHub Pages。**
> 本节内容保留作以后参考；真要迁的时候注意：Cloudflare 侧不用设 `PUBLIC_BASE_PATH`（根路径），
> 但要显式设 `PUBLIC_SITE_URL`，否则 canonical / sitemap 会指向 github.io。
